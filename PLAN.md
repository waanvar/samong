...
2. ถ้าพอร์ตถูกยึดโดย**คนอื่น** → เลื่อนขึ้น (ดีฟอลต์ย้ายจาก 3000 มาเป็น 3117 แล้ว
   เพราะ 3000 เป็นของ Node/Rails/Grafana จนพอร์ตแรกมักไม่ว่างและ URL ขยับทุกครั้ง)
...
> หนี้ก้อนแรกจ่ายแล้วใน Phase 31 (v0.4.0) ส่วน code signing ยังอยู่ แต่**เตี้ยลงกว่า
> ที่ประโยคข้างบนประเมินไว้**: Homebrew, Scoop และ `cargo install` ไม่ผ่าน prompt
> ของ OS เลยสักทาง ทางดับเบิลคลิกจึงไม่ใช่ประตูเดียวอีกต่อไป — ดู
> `docs/RELEASE-LESSONS.md` §7

## Phase 31 — ของที่ดับเบิลคลิกได้แล้ว แต่ยังดูเหมือนยังไม่เสร็จ (เพิ่มหลัง Phase 30)

v0.4.0 — ไอคอน, AUR, และเอกสารที่กลายเป็นเท็จ

**ไอคอนไม่ใช่เรื่องความสวย มันคือคำตอบของคำถามว่า "นี่เป็นโปรแกรมจริงหรือเปล่า"**
Phase 30 จบด้วย launcher ที่ทำงานถูกทุกอย่างแล้วโผล่มาเป็นสี่เหลี่ยมเปล่าใน
taskbar · บนวินโดวส์ resource ใน `.exe` คือ**ที่เดียว**ที่ Explorer, taskbar และ
กล่อง SmartScreen มอง จะวางไฟล์ `.ico` ไว้ข้างๆ กี่ไฟล์ก็ไม่มีอะไรอ่าน จึงต้องมี
`build.rs` · macOS ใช้ `.icns` ใน bundle, Linux ใช้ hicolor PNG กับ `Icon=samong`
ใน desktop entry · ทั้งหมดสร้างจากต้นฉบับเดียวด้วย `packaging/icons/make-icons.py`

**สองชิ้นงาน ไม่ใช่หนึ่ง**
ต่ำกว่าราว 24px เส้นสี่เส้นกับจุดสี่จุดของ mark เหลือน้อยกว่าพิกเซลละอัน แล้ว
render ออกมาเป็นหมอกเทา · ขนาดเล็กจึงใช้ภาพที่ลดรูป — หนึ่งจุด หนึ่งเส้น จุดที่
สว่าง — และ mark เต็มเข้ามาแทนตั้งแต่ 32px ขึ้นไป

**ไม่เชื่อว่า `build.rs` ทำงาน — อ่านกลับออกมาจาก PE**
`packaging/icons/check-exe-icon.py` นับ `RT_ICON` ในไฟล์ที่ build ออกมาจริง
เป็นด่านชนิดเดียวกับ `verify-stage.sh`: สิ่งที่ตรวจคือ artefact ไม่ใช่เจตนา

- [x] **AUR `samong-bin`** — CI สร้างในคอนเทนเนอร์ Arch แล้ว**ตรวจสิ่งที่มันมีไว้เพื่อ**
      ไม่ใช่ตรวจว่าติดตั้งผ่าน: `namcap` ทั้ง PKGBUILD และแพ็กเกจ, `pacman -U`,
      ทุกไบนารีอยู่บน `PATH`, แล้วค้นจริง resolve wikilink จริง และตัดประโยคไทยจริง
      — ข้อสุดท้ายผ่านก็ต่อเมื่อ dictionary ถูกคอมไพล์เข้าไปในไบนารีแล้วเท่านั้น
- [x] **namcap คืน exit 0 เสมอ** บรรทัด `E: Dependency hicolor-icon-theme detected
      and not included` จึงผ่านเงียบทุกรอบขณะที่ job เขียว · เจอเพราะบังเอิญไล่ log
      ตอนตรวจเรื่องอื่น · ตอนนี้ `E:` ล้ม ส่วน `W:` พิมพ์ให้เห็นแล้วปล่อยผ่าน เพราะ
      สิ่งที่มันเตือนถูกต้องสำหรับแพ็กเกจ `-bin` (ไบนารีตั้งใจไม่ strip เพื่อให้ยัง
      ตรงกับ checksum ที่เผยแพร่)
- [x] crate เลิกแบกไอคอน — `include` เขียนว่า `/assets/**/*` ซึ่งถูกตอนที่ในนั้นมีแต่
      dictionary ภาษาไทย แต่ตอนนี้จะพา 1.16 MB ที่ไม่มี `include_str!` ตัวไหนอ่านไปด้วย
- [x] `CODE_OF_CONDUCT.md`, issue form และ PR template · ช่องบังคับของ bug form คือ
      คำถามที่กลายเป็นคำตอบของบั๊กจริงมาแล้ว: ติดตั้งด้วยวิธีไหน เวอร์ชันอะไร และ
      vault เปล่าทำซ้ำได้ไหม
- [x] `.gitattributes` ตรึงไฟล์ที่ CR เปลี่ยน**พฤติกรรม** ไม่ใช่หน้าตา — `PKGBUILD`,
      `.SRCINFO`, `*.sh`

**เอกสารที่กลายเป็นเท็จ คือบั๊กชนิดหนึ่ง**
`packaging/README.md` ไปอยู่ในทุก archive ในชื่อ `DOUBLE-CLICK.md` มันจึงเป็นสิ่งที่
คนที่เพิ่งเจอ SmartScreen เปิดอ่าน — และมันเขียนว่าทางแก้คือเงิน ซึ่งไม่จริงตั้งแต่
2024: SmartScreen ให้ reputation ตามยอดดาวน์โหลด ไม่ใช่ตามการมีใบรับรอง แม้เป็น EV
และ reputation นั้นรีเซ็ตเมื่อต่ออายุใบรับรอง ใบรับรองเปลี่ยน "Unknown publisher"
ให้เป็นชื่อ แต่ไม่ได้เอาคำเตือนออก · ทางที่ไม่เจอ prompt เลยมีอยู่แล้วและฟรี
เอกสารทั้งสามฉบับจึงนำด้วยทางนั้น

**บั๊กสามตัวใน release pipeline ที่รอบนี้เผยออกมา** (รายละเอียดใน `docs/RELEASE-LESSONS.md`)
1. `publish` job รัน `cargo package --list --locked` โดยไม่มี `--allow-dirty` จึงล้ม
   ทุก release ที่ `web/dist` ซึ่งเป็นของที่ตั้งใจใส่เข้า crate — **นี่คือเหตุผลจริง
   ที่ crates.io ต้องปล่อยด้วยมือมาตลอด ไม่ใช่เพราะไม่มี token**
2. `winget` job สร้าง manifest ใหม่แล้วรัน `--check` เทียบกับไฟล์ที่ commit ไว้ ซึ่งยัง
   เป็นเวอร์ชันก่อนหน้า และเป็นการตรวจที่ผ่านไม่ได้เลยตอน release เรื่อง drift เป็น
   งานของ CI ที่เทียบกับสิ่งที่ commit ไว้
3. `generate.py` ทุกตัวถาม `api.github.com` โดยไม่ยืนยันตัวตน = 60 ครั้งต่อชั่วโมง
   ต่อ IP บน runner ที่แชร์กัน ทำให้ tap bump ล้มติดกันห้ารอบด้วย 403 ที่ดูเหมือน
   บั๊กของสิ่งที่รันมัน · ใส่ `GITHUB_TOKEN` เฉพาะที่ `api.github.com` เพราะ URL ของ
   release asset redirect ไป `objects.githubusercontent.com` ซึ่งปฏิเสธ
   `Authorization` ที่มันไม่ได้คาดไว้

**Acceptance**: โหลด archive ที่ publish จริงมาแกะ — checksum ตรงกับ `.sha256`,
ไบนารี Windows ทั้งห้ารวม `Open Samong.exe` มี `RT_ICON` ครบเจ็ดรายการพร้อม version
field, archive ของ Linux มี hicolor ครบแปดขนาดกับ `samong.desktop`

## Phase 32 — พอร์ตที่อยู่กับที่ (เพิ่มหลัง Phase 31)

v0.4.1 — ค่าเริ่มต้นที่ไม่ขยับ และ CI ที่สั่งรันเองได้

**พอร์ตที่ขยับทุกครั้งที่เปิด = บุ๊กมาร์กที่ใช้ไม่ได้**
3000 เป็นของ Node, Rails, Grafana และ dev server ที่ scaffold มาแทบทุกตัว บนเครื่อง
นักพัฒนาพอร์ตแรกจึงมักไม่ว่าง launcher ก็ไล่ขึ้นไปตามที่ Phase 30 ออกแบบไว้ ซึ่ง
**ทำงานถูกต้องและให้ผลที่ใช้ไม่ได้**: URL ย้ายที่ระหว่างการรัน · 3117 อยู่ในช่วง
registered-but-unassigned · `--port` และ `SAMONG_PORT` ไม่เปลี่ยน ใครติดของเดิมใช้
`--port 3000` ได้

- [x] `samong-server --port` อ้างค่าคงที่ตัวเดียวกับที่ launcher ใช้ค้นหา เดิมเป็น
      ตัวเลขสองตัวคนละที่ที่บังเอิญตรงกัน ถ้าเพี้ยนเมื่อไหร่ launcher จะไปเคาะพอร์ต
      ที่ server ไม่เคยฟัง และอาการที่ผู้ใช้เห็นคือ "ดับเบิลคลิกแล้วไม่มีอะไรขึ้น"
      ซึ่งไล่หายากที่สุดในบรรดาอาการทั้งหมดของโปรแกรมนี้
- [x] **`workflow_dispatch` พร้อม input `ref`** — `push` เคยเป็นทางเข้าเดียวของ
      `ci.yml` ซึ่งพอใช้ได้จนกระทั่ง GitHub Actions มี major outage แล้ว `cd184b8`
      ลงไปอยู่บน `main` **โดยไม่มี run ผูกอยู่เลย** ไม่ใช่ queued ไม่ใช่ล้ม คือไม่มี ·
      ถ้าไม่มีปุ่มนี้ ทางเดียวที่จะขอ run ให้คอมมิตนั้นคือ push คอมมิตเปล่า · `ref`
      ต่อสายเข้า `actions/checkout` ทั้งหกงานจริง ไม่งั้นการสั่งรันด้วย SHA จะไป
      checkout branch แทนอย่างเงียบๆ แล้วรายงานผลของคอมมิตอื่น ซึ่งแย่กว่าไม่มีปุ่ม

**Acceptance**: รันไบนารีที่ปล่อยออกไปจริงแล้ววัดจากภายนอก — log บอก
`listening on http://127.0.0.1:3117`, พอร์ต 3117 เปิด, พอร์ต 3000 ปิด ·
`vite` proxy ทดสอบแบบต่างระดับ (server เปิด = 200, server ปิด = 500 ซึ่งพิสูจน์ว่า
มันวิ่งไป 3117 จริง ถ้ายังชี้ 3000 จะได้ 500 ทั้งสองกรณี) · **ค่าคงที่ในซอร์สที่
เปลี่ยนไม่ใช่หลักฐานว่าไบนารีที่ส่งออกไปผูกพอร์ตนั้น การรันมันต่างหากที่เป็น**
