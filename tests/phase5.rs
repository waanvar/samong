...
async fn serves_spa_with_index_fallback_and_api() {
    ...
    fs::write(
        ui.join("index.html"),
        "<!doctype html><title>Banyan</title>",
    )
    .unwrap();
    fs::write(ui.join("assets").join("app.js"), "console.log('banyan')").unwrap();
    ...
    assert!(index.text().await.unwrap().contains("Banyan"));
    ...
    assert!(fallback.text().await.unwrap().contains("Banyan"));
