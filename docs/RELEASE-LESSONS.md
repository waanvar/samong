...
| v0.5.0 | **`samong eval` printed Thai as `\u{e31}` escapes.** The report and the answer-key error used `{:?}`, and Rust's Debug escapes grapheme-extended characters — nearly every Thai vowel and tone mark. The project's whole reason to exist is Thai search, and the one line a reader has to recognise to act on a miss was the line that could not be read. |
| v0.5.0 | **Every documented similarity floor was below the whole score distribution.** `docs/EVAL.md`, the README and the flag's help all suggested 0.2–0.4; `multilingual-e5-small` scores unrelated text at about 0.75. The floors filtered nothing, and a sweep whose rows are all identical reads as "this feature is inert" rather than "your range is wrong". Nobody could have noticed before the first real sweep, because nothing prints the scores. |
