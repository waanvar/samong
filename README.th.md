...
**ฐานความรู้แบบ local-first สำหรับโน้ตที่อยู่ใน repository ของคุณอยู่แล้ว**

คุณแก้ปัญหานั้นได้แล้วครั้งหนึ่งและจดไว้ ครึ่งปีต่อมาโน้ตยังอยู่ใน repo แต่หาไม่เจอ
ชี้ Samong ไปที่ root ของโปรเจกต์ แล้วไฟล์ `.md` ที่คุณจะ commit จะกลายเป็นสิ่งที่
ค้นได้และเชื่อมโยงกัน — และ AI ที่คุณใช้อยู่ก็อ่านได้ · Markdown ธรรมดา เข้ากันได้กับ
[Obsidian](https://obsidian.md) (`[[wikilink]]` / `[[wikilink|alias]]`)
ไม่ต้องมีบัญชี ไม่มีคลาวด์ ไฟล์เดียวจบ

*[English version →](README.md)* — ฉบับหลักคือภาษาอังกฤษ ฉบับนี้เป็นคำแปล

![Samong — กราฟคือพื้นที่ทำงาน เห็นชื่อโน้ต ขนาด hub และการจับกลุ่มตามโฟลเดอร์](docs/graph-dark.png)

## ทำไมต้อง Samong

- 📁 **โน้ตคือไฟล์ `.md` ที่คุณจะ commit** ชี้ไปที่ repository ได้เลย ไม่ต้องมี
  โฟลเดอร์โน้ตพิเศษ · เคารพ `.gitignore` และข้ามโฟลเดอร์ dependency เสมอ
  root ของโปรเจกต์จึง index ADR, runbook, เอกสารออกแบบ ได้โดยไม่ดูด
  `node_modules` เข้ามา — และ `samong doctor` บอกว่าอะไรถูกนับ อะไรไม่ถูกนับ
- 🤖 **โน้ตของคุณกลายเป็นความจำของ agent** `samong-mcp` พูด MCP ได้ Claude Code
  และ Claude Desktop จึงค้น อ่าน และเขียนฐานความรู้ชุดเดียวกับคุณ
  ไม่ต้องแปะ context กลับเข้าไปทุกรอบ ([วิธีตั้งค่า](docs/AI-AGENT.md))
- 🗺 **กราฟคือพื้นที่ทำงาน ไม่ใช่แท็บของแปลก** vault คือรูปร่างหนึ่ง พิมพ์แล้ว
  ทุกอย่างที่ไม่ตรงจะหมองลง คำค้นจึงกลายเป็นสถานที่
- 🔎 **อ้างโน้ตด้วย path ไม่ใช่ title** repo เดียวมีไฟล์ชื่อ `README.md` ได้ยี่สิบไฟล์
  title แยกมันไม่ออก และ index ที่ key ด้วย title จะยุบมันรวมกันเงียบๆ — ผลค้นหา
  API และ MCP จึงคืน path จริงมาให้ทุกครั้ง
- 📚 **เรียนรู้จากเอกสารที่ไม่ได้ commit** `.gitignore` ตอบว่า "จะแจกอะไร"
  แต่ฐานความรู้ต้องตอบว่า "จะเรียนรู้จากอะไร" · `scope.include` ดึงเอกสารของ
  dependency เข้ามาเป็นโน้ตอ่านเท่านั้น — หนึ่งโปรเจกต์ หนึ่งสมอง ไม่ต้องแยก vault
- 🔗 **หนึ่ง repo หนึ่ง vault แต่ลิงก์ถึงกันได้ทั้งหมด** `[[ชื่อ-vault/ชื่อโน้ต]]`
  ชี้ข้าม vault ที่ลงทะเบียนไว้ได้ และ backlinks ย้อนกลับมาโดยไม่ต้องมี index
  ข้าม vault ให้ดูแล
...
- 🧠 **ค้นด้วยความหมาย เป็นตัวเลือก และรันในเครื่อง** build ด้วย
  ...
  ด้วยโมเดล multilingual ที่ไม่ออกจากเครื่องคุณ · ปิดไว้เป็นค่าเริ่มต้นโดยมีเหตุผล
- ⚡ **เร็ว และเป็นของคุณ** link graph ใน [redb](https://github.com/cberner/redb),
  ค้นหาด้วย [tantivy](https://github.com/quickwit-oss/tantivy),
  incremental reindex แตะเฉพาะไฟล์ที่เปลี่ยน · index ทั้งหมดอยู่ใน
  `<vault>/.brain/` และสร้างใหม่จากไฟล์ `.md` ได้ทุกเมื่อ ไม่มีอะไรผูกขาคุณไว้
